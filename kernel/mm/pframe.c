...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
    ...
    *result = pframe_get_resident(o, pagenum);
    if(*result == NULL){
        /*Need to call pageoutd??? Need to fix???*/
        dbg(DBG_PRINT, "GRADING3B 1\n");
        /*if (pageoutd_needed()){
            dbg(DBG_PRINT, "GRADING MARCO 1001\n");
            pageoutd_wakeup();
        }*/
        ...
        *result = pframe_alloc(o, pagenum);
        ...
        int pframe_fill_retval = 0;
        if ((pframe_fill_retval = pframe_fill(*result)) < 0){
            dbg(DBG_PRINT, "GRADING3D 2\n");
            pframe_free(*result);
            return pframe_fill_retval;
        }

    }
/*    else{
        dbg(DBG_PRINT, "GRADING3B 1\n");
        while(pframe_is_busy(*result) != 0){
            dbg(DBG_PRINT, "GRADING MARCO 1005\n");
...
pframe_pin(pframe_t *pf)
{
    ...
    if ( !pframe_is_pinned(pf)){
        dbg(DBG_PRINT, "GRADING3B 1\n");
        if (list_link_is_linked(&(pf->pf_link))){
            list_remove(&(pf->pf_link));
            dbg(DBG_PRINT, "GRADING3B 1\n");
