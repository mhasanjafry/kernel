...
do_brk(void *addr, void **ret)
{
		...
		if ( ADDR_TO_PN(PAGE_ALIGN_UP(addr)) < ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk))){
			...
			void *j = page_alloc_n(npages);
			memset((void *)j, 0, (size_t)(npages*PAGE_SIZE));
			vmmap_write(curproc->p_vmmap, PAGE_ALIGN_UP(addr), j, (size_t)(curproc->p_brk) - (size_t)PAGE_ALIGN_UP(addr));
			page_free_n(j, npages);
			...
			if (!vmmap_is_range_empty(curproc->p_vmmap, ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk)), npages)){
	            dbg(DBG_PRINT, "GRADING3D 2\n");
				return -ENOMEM;
			}
			vmarea_t * vm_new = vmmap_lookup(curproc->p_vmmap, ADDR_TO_PN(PAGE_ALIGN_UP(curproc->p_brk))-1);


			vm_new->vma_end = ADDR_TO_PN(PAGE_ALIGN_UP(addr));
		}
		curproc->p_brk = addr;
		*ret = addr;
        dbg(DBG_PRINT, "GRADING3B 1\n");
