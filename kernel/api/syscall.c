...
sys_read(read_args_t *arg)
{
    ...
    copy_from_user(&kernel_args, arg, sizeof(read_args_t));
    
    void *buf = page_alloc();
    /*if (NULL == (buf = page_alloc())){
        dbg(DBG_PRINT, "GRADING MARCO 1012\n");
        return -1;
    }*/
    read = do_read(kernel_args.fd, buf, kernel_args.nbytes);
    if(read < 0)
    {
        page_free(buf);
...
sys_write(write_args_t *arg)
{
    ...
    copy_from_user(&kernel_args, arg, sizeof(write_args_t));

    void *buf = page_alloc();

    copy_from_user(buf, arg->buf, arg->nbytes);

    write = do_write(kernel_args.fd, buf, kernel_args.nbytes);
    if(write < 0)
    {
        dbg(DBG_PRINT, "GRADING3D 1\n");
        page_free(buf);
        curthr->kt_errno = -write;
        return -1;
    }
    page_free(buf);
    dbg(DBG_PRINT, "GRADING3B 1\n");
...
sys_getdents(getdents_args_t *arg)
{
    ...
    copy_from_user(&kernel_args, arg, sizeof(getdents_args_t));

    dirent_t buf;
    int num_dirents = (int)(kernel_args.count / sizeof(dirent_t));
    int i = 0;
    for(; i < num_dirents; i++){
        if((err = do_getdent(kernel_args.fd, &buf)) < 0){
            dbg(DBG_PRINT, "GRADING3D 1\n");
            curthr->kt_errno = -err;
            return -1; 
        }
        ...
        copy_to_user((void *) (kernel_args.dirp+i), &buf, sizeof(dirent_t)); 

        dbg(DBG_PRINT, "GRADING3B 1\n");
    }
    dbg(DBG_PRINT, "GRADING3B 1\n");
    return (i * sizeof(dirent_t));
}
