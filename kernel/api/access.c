...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
    ...
    vmarea_t * vm_new = vmmap_lookup(p->p_vmmap, ADDR_TO_PN(vaddr));
    if (vm_new == NULL){
        dbg(DBG_PRINT, "GRADING3D 1\n");
        return 0;
    }
    /*if ((perm | vm_new->vma_prot) != vm_new->vma_prot){
        dbg(DBG_PRINT, "GRADING MARCO 1004\n");        
        return 0;
    }*/
    return 1;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    size_t curr = (size_t)avaddr;
    dbg(DBG_PRINT, "GRADING3B 1\n");
    
    while (curr < (size_t)avaddr + len){
        dbg(DBG_PRINT, "GRADING3B 1\n");
        if (!addr_perm(p, (const void *) curr, perm)){
            dbg(DBG_PRINT, "GRADING3D 1\n");
            return 0;
        }
